...
def get_positions(**kwargs):
    ...
    url_stream = f"https://integrate.us.mixtelematics.com/api/positions/assets/from/{formatted_date}000000/to/{formatted_date}235959"
    ...
    response = post(url_stream, headers=headers, json=payload)
...
def transmission_gcp(**kwargs):
    ...
    df_positions = pd.DataFrame.from_dict(data)
    #Transmitindo os dados por BytesIO
    buffer = BytesIO()
    df_positions.to_parquet(buffer, index=False)
    buffer.seek(0)
    #Enviando para GCP
    gcs_hook = GCSHook(gcp_conn_id='gcp')
    gcs_hook.upload(
        bucket_name=hp_gcp_bucket_name_raw,
        object_name=f"mix/positions/positions_{formatted_date}.parquet",
...
def insert_dag_metadata(**kwargs):
    ...
    gcs_hook.upload(
        bucket_name=hp_gcp_bucket_name_raw,
        object_name=f"mix/metadata/positions/pipeline_hp_telemetics_positions_{formatted_date}.json",
...
def atualizar_tabela_bigquery(**kwargs):
    ...
    gcs_to_bigquery = GCSToBigQueryOperator(
        ...
        source_objects=[f"mix/positions/*.parquet"],
        destination_project_dataset_table='gcp-hp-proj-operacao.dev.positions',
...
def pipeline_hp_mix_telemetics_positions():
