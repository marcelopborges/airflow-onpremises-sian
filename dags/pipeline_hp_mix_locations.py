...
def get_locations(**kwargs):
    ...
    url_stream = f"https://integrate.us.mixtelematics.com/api/locations/group/{hp_mix_group_id}"
...
def transmission_gcp(**kwargs):
    location_json = kwargs['ti'].xcom_pull(task_ids='get_locations')
    ...
    buffer = BytesIO()
    df_location.to_parquet(buffer, index=False)
    buffer.seek(0)
    #Enviando para GCP
    gcs_hook = GCSHook(gcp_conn_id='gcp')
    gcs_hook.upload(
        bucket_name=hp_gcp_bucket_name_raw,
        object_name=f"mix/location/location.parquet",
        data=buffer.getvalue(),
        mime_type='application/octet-stream'
    )

    logging.info(f"Arquivo Parquet enviado para o bucket GCP: location.parquet")
...
def atualizar_tabela_bigquery(**kwargs):
    ...
    gcs_to_bigquery = GCSToBigQueryOperator(
        ...
        source_objects=[f"mix/location/*.parquet"],
        destination_project_dataset_table='gcp-hp-proj-operacao.dev.location',
